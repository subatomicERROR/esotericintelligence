...
import { ContentCategory, type PageProps, type ContentItem } from '../types';
...
const QuantumPage: React.FC<PageProps> = ({ purchasedItems, onPurchase, showToast, content }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [sortOrder, setSortOrder] = useState('newest');

  const filteredAndSortedContent = useMemo(() => {
    let items = content.filter(
      item => item.category === ContentCategory.Quantum && item.published
    );
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Explore the intersection of quantum physics, reality, and the nature of the mind."
      />
      <StoreControls 
