...
const SpiritualPage: React.FC<PageProps> = ({ purchasedItems, onPurchase, showToast, content }) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [sortOrder, setSortOrder] = useState('newest');

  const filteredAndSortedContent = useMemo(() => {
    let items = content.filter(
      item => item.category === ContentCategory.Spiritual && item.published
    );
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Delve into ancient truths, mystical poetry, and timeless insights on the human spirit."
      />
      <StoreControls 
